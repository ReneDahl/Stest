...
passport.use(new FacebookStrategy({
    ...
    (accessToken, refreshToken, profile, done) => {
        ...
        const token = jwt.sign(payload, secret, { expiresIn: '1h' });
        done(null, {email: userEmail, username: profile.name.givenName, token: token});
    })
);
...
passport.use(new GoogleStrategy({
    ...
    function(accessToken, refreshToken, profile, done) {
        ...
        const payload = { username: userEmail };
        const token = jwt.sign(payload, secret, { expiresIn: '1h' });
        done(null, {email: userEmail, username: profile.displayName, token: token});
    }
));
...
passport.use(new twitchStrategy({
    ...
    function(accessToken, refreshToken, profile, done) {
        ...
        const payload = { username: userEmail };
        const token = jwt.sign(payload, secret, { expiresIn: '1h' });
        done(null, {email: userEmail, username: profile.display_name, token: token});
    }
));
